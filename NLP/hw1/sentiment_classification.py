...
def import_weight_csv(file_path):
    ...
    with open(file_path, 'r') as f:
        reader = csv.reader(f)
        for each_line in reader:
            weight.append(each_line)
    return weight

def convert_data_to_tfidf(doc, max_feature_cnt):
    # init the vectorizer
    vectorizer = TfidfVectorizer(max_features=max_feature_cnt)
...
def fit_gd(X_train, y_train, eta=0.01, n_iters=30000, epsilon=1e-5):
    print("Start fitting...")
